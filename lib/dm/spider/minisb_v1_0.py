...
class MiniSpider:
    def __init__(self, url_db_dir, result_dir,  entry_list,proxy_dir, white_list = [], black_list = [], download_list = [], piror_mode = 0, 
                thread_num = 1, time_out = 1, time_sleep = 1, queue_max_size = 1e6, url_max_size = 1e7, file_max_size = 1e10, debug = 0,run_mode = 0):
    ...
    def crawl_url(self, url , proxy_ip_port):
		#定义一个字典
        ...
        #代理ip如何使用，如下
        proxy_ip = {'http':proxy_ip_port}   #proxy_ip = {'http':'122.224.227.202:3128'}  
        ...
        while retry < 10:
            try:#尝试读取网页数据 对目标url访问
                ...
                #如果访问失败，判断是否是代理的问题，即访问百度，如果失败，则认为代理失败
                try:
    ...
    def run(self):
        ...
        if self.run_mode == 0:
            for tid in range(self.thread_num):
                t = threading.Thread(target = self.thread_work_mode0, args = (tid, ))  #target是一个可调用的对象
                                                                             #调用target时的参数列表和关键词列表
                threads.append(t)
        
        if self.run_mode == 1:
            for tid in range(self.thread_num):
                t = threading.Thread(target = self.thread_work_mode1, args = (tid, ))  #target是一个可调用的对象
                                                                             #调用target时的参数列表和关键词列表
                threads.append(t)
